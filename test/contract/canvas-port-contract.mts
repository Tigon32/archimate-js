...
export const CANVAS_PORT_CONTRACT_EXCLUDED_CASES = [
  'connect/reconnect/label/delete gestures remain covered by targeted DiagramJsCanvasPort tests until every port exposes them.',
  'batch multi-node move is asserted only as an unsupported atomic gesture; EE-M5 adds first-class batch commands.'
];
...
export type CanvasPortHarness = {
  ...
  emitUnsupportedMultiMove(ids: string[]): unknown;
...
function rejectedCase(options: CanvasPortContractOptions): ContractCase['run'] {
  return (assert) => withAttached(options, assert, ({ editor, harness }) => {
    ...
    assert.throws(() => harness.emitUnsupportedMultiMove(['node-component', 'node-service']));
