...
`synthetic/meff-core-candidate.xml` is a second, independently hand-authored synthetic fixture shaped around the public MEFF overview's model/name/elements core, with a relationship added to probe this parser's current support. It is not copied from an official example and is also not XSD-validated. The importer contract test observes model ID/name round-tripping, while the public import path reports unmapped element entries and generic relationships with unresolved endpoints through stable warnings. It does not establish MEFF conformance or interoperability.
