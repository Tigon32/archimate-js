...
assert.deepEqual(Object.keys(packageJson.exports).sort(), ['.', './app-shell.css', './export', './layout', './lint', './model-dto', './validator'],
...
assert.match(exportsDoc, /archimate-js\/lint/);
assert.match(exportsDoc, /archimate-js\/export/);
...
const checks = [
  ...
  ['Node export API browser contract', ['run', 'test:export-api']],
  ['CLI browser export contract', ['run', 'test:cli:browser']],
  ['packed external consumer contract', ['run', 'test:packed-consumer']]
];
