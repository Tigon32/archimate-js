...
describe('synthetic performance benchmark foundation', () => {
  it('generates stable, synthetic semantic and diagram fixtures', () => {
    ...
    const second = createSyntheticFixtures(9);
  ...
  it('accepts the versioned result shape and rejects non-synthetic output', () => {
    const result = {
    ...
    expect(validateBenchmarkResult({
      ...
      benchmarks: [{
        ...result.benchmarks[0],
        fixture: {
          ...result.benchmarks[0].fixture,
          diagram: { nodeCount: 2, connectionCount: 0 }
        }
      }]
    })).toBe(false);
    expect(validateBenchmarkResult({
