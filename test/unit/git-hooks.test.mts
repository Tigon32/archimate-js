...
describe('local verification gate', () => {
  ...
  it('installs hooks and orders deterministic checks before remote escalation', () => {
    ...
    expect(packageJson.scripts['verify:wip']).toBe('run-s check:source-policy test:relationship-matrix test:relationship-projection test:legacy-relationship-util test:concept-renderer lint test:typecheck test:validator-build');
