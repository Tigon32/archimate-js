...
Direct npm dependencies are declared in [`package.json`](package.json) and installed as separate packages, not copied into this repository. Their declared licenses are: MIT (`css.escape`, `diagram-js`, `diagram-js-direct-editing`, `eslint-plugin-bpmn-io`, `eslint-plugin-import`, `hex-and-rgba`, `ids`, `min-dash`, `min-dom`, `moddle`, `moddle-xml`, `tiny-svg`, and the listed development tools); ISC (`inherits`, `inherits-browser`, `saxes`); Apache-2.0 (`mathjs`, `typescript`); OFL-1.1 (`archimate-font`); and EPL-2.0 (`elkjs@0.12.0`). For `elkjs`, this repository selects the EPL-2.0 path from upstream's `EPL-2.0 OR GPL-3.0-or-later` expression and declines the GPL secondary-license option. The full EPL-2.0 text is included at [`licenses/elkjs-EPL-2.0.txt`](licenses/elkjs-EPL-2.0.txt) and shipped in the npm package. Upstream source and package provenance: [`kieler/elkjs` tag 0.12.0](https://github.com/kieler/elkjs/tree/0.12.0), [`npm package`](https://www.npmjs.com/package/elkjs). This is a direct-dependency inventory; CI `npm audit` covers the resolved dependency tree.
