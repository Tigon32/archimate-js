...
| EE-M10 Smart relationship chooser & quick-create — Implemented in #352 | Add valid relationship chooser and drag-to-empty quick-create. | Relationship semantics stay domain-owned. | Relationship UI, rules, commands. | `src/modeler/relationship-chooser.ts`, rule adapter, DTO commands. | EE-M5, EE-M7, #333, #102. | Unit decision/chooser tests and browser integration coverage. | Chooser appears for multiple valid types; unambiguous default works; unsupported/disallowed are distinct; quick-create uses deterministic DTO commands. | Reviewed profile is 23 rows and incomplete; no silent fallback; profile expansion remains #102. | L |
