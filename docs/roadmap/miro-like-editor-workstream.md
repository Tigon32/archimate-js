...
| EE-M2 Engine contract test suite (headless fake port + DiagramJsCanvasPort) | Define reusable engine/port behavior tests. | Make adapters interchangeable by contract. | DTO editor, adapter, tests. | `test/unit`, `test/browser`, `src/model-dto`. | EE-M1. | Headless fake port plus browser port tests. | Render, command, selection, detach, undo/redo behavior is covered. | Must not require React or new engine. | M |
