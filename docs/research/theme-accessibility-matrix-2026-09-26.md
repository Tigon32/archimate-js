...
| Contrast | Rendered text, controls, status, hover, invalid boundaries, focus outline | `docs/research/theme-contrast-browser-2026-09-24.md` reports minimum text/boundary/focus ratios. Lowest ordinary text is 7.79:1; lowest high-contrast text is 10.28:1; lowest boundary is 4.38:1; lowest focus is 6.05:1. Pending modeler-specific contrast evidence is tracked in `docs/research/modeler-theme-contrast-2026-09-26.md` from sibling #389. | Ratios are computed sRGB values for listed states, not a full-page WCAG conformance claim. |
...
| Reduced motion | App shell animation/transition suppression | `assets/design-tokens/app-shell.css` includes `@media (prefers-reduced-motion: reduce)` to reduce app-shell animations and transitions. Sibling #390 owns the current reduced-motion browser evidence. | This matrix records the contract and sibling dependency; it does not duplicate #390's persistence-section edits. |
