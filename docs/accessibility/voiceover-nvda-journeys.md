...
Export the same synthetic view using the generic procedure's `compile:cli`
command and open the resulting `synthetic-showcase.svg` in Safari. Read it as a
static document. Record whether VoiceOver exposes the SVG root's document name
and description and whether meaningful objects expose their names and roles.
...
Export the same synthetic view using the generic procedure's `compile:cli`
command and open `synthetic-showcase.svg` in Firefox. In browse mode, use `g`
to move to the next graphic if NVDA exposes the SVG as a graphic; use
