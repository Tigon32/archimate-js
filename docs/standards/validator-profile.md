...
4. **Standard semantics:** targets ArchiMate 3.2 and recognizes the current core element and relationship vocabularies. A pure tri-state service applies a small, versioned decision set and reports explicit disallowed rows as errors. Unlisted combinations, junctions, and relationship endpoints are reported as unsupported. It does not derive semantic truth from editor menu maps and does not claim a complete normative source/relationship/target matrix.
...
The Open Group ArchiMate Specification is normative for language semantics. ArchiMate 4 was released in April 2026; this validator profile and its current semantic rules still target ArchiMate 3.2. The Open Group's [licensed-downloads page](https://www.opengroup.org/archimate-licensed-downloads) lists ArchiMate 4 and its personal evaluation route. The [Model Exchange File Format resources](https://www.opengroup.org/xsd/archimate/) and official schema artifacts are the authority for exchange shape; official schema files are not bundled here because redistribution terms have not been established. The built-in checks are intentionally described as a subset, not XSD validation or conformance.

If a consumer finds behavior that appears inconsistent with the specification or official exchange artifacts, use the [project's conformance reporting guidance](../rendering/read-only-html-embed.md#reporting-suspected-standards-gaps). Documented unsupported checks should be reported as limitations unless new evidence changes their scope.
