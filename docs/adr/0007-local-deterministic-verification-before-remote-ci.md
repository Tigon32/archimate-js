...
Policy clarification (2026-09-25): Accepted ADR-0005 supersedes this ADR's original `agent/*` automatic-drain eligibility. The drain implementation rejects agent PRs until independent approval is enforced by branch protection and the policy and tests are updated together. This ADR's automatic drain remains limited to explicitly allow-listed Dependabot groups; see #264 and follow-up task #319.
...
9. Queue draining is repository-owned and opt-out, not armed per PR. A trusted workflow on `main` reacts to completed pull-request workflows and binds its decision to exact HEAD. It may merge only explicitly allow-listed grouped Dependabot minor/patch version/security PRs after `CI` and `Automated security analysis` succeed and all exact-HEAD PR workflows complete successfully. Eligibility additionally requires the actual `dependabot[bot]` author and an allow-listed group branch; major, ungrouped, and unknown dependency updates remain manual. Agent PRs are ineligible for automatic drain under ADR-0005 until independent approval is enforced at merge time and the drain policy and tests are updated together. The `no-auto-merge` label is the emergency brake for otherwise eligible candidates. The workflow never checks out PR code with its write-capable token.
...
12. Remote verification is itself cost-ordered. One primary supported runtime executes the complete canonical qualification first. Secondary runtime/platform lanes MUST NOT repeat the whole suite merely for symmetry; they run the smallest checks capable of detecting their distinct compatibility risk, and expensive secondary lanes SHOULD depend on primary success unless their evidence is orthogonal and time-sensitive.
13. The current profile uses Node 24 as the full primary qualification. Node 22 runs package/CLI/packed-consumer compatibility only after Node 24 succeeds. Browser-only CLI/performance evidence is collected in the Node 24 job rather than a second duplicate browser job. macOS is justified by the canonical temporary-path regression in issue #161, but that evidence supports a path-scoped lane for relevant CLI/path changes plus a weekly drift check—not a macOS runner on every PR.
14. Orthogonal security and standards checks may run concurrently with primary qualification when they provide distinct evidence and do not duplicate the same expensive workload. Local and remote gates may overlap intentionally where the trust boundary requires independent evidence.
15. Automated maintenance generation does not imply parallel maintenance execution. Failed or manual-review Dependabot PRs feed one serialized dependency-maintenance queue. Agents MUST claim the persistent queue issue rather than individual bot PRs; a live queue lease freezes selection, and unselected bot PRs remain suppressed until the owner releases the queue. Green allow-listed minor/patch updates bypass this queue and continue through automatic drain.
16. The dependency queue selects deterministically (oldest blocked/manual PR first) and records the failing workflow fingerprint plus suppressed waiting PRs. This prevents multiple agents from repeating the same diagnosis or editing overlapping dependency/policy files while preserving visibility of the backlog.
17. Do not claim a fixed percentage of failures caught locally without measurements. Track WIP-gate failures, full-local failures, cancelled superseded background runs, reusable exact-HEAD verification receipts, remote failures that were locally reproducible, Actions consumption, and repeated push/CI cycles when the data is available.
...
- A green Ready `agent/*` PR still requires independent human review and is not automatically drained under ADR-0005; eligibility can be reconsidered only after branch protections enforce approval and the drain policy/tests change together.
- The drain policy is default-on only for explicitly allow-listed grouped Dependabot minor/patch lanes; it is default-off for agent and human branches, forks, majors, and ungrouped/unknown dependency updates, with `no-auto-merge` as an explicit stop control.
- Red/manual Dependabot work cannot fan out across agents: one persistent queue issue is the only dependency-maintenance claim surface.
...
- Remote-only platform, security, standards, and release checks continue to consume CI when their distinct evidence justifies the cost. Platform diversity is sampled proportionately rather than duplicated on every change.
- macOS cost is bounded to the path-safety failure class that previously escaped Linux, plus scheduled drift detection.
...
- Automatic queue-drain dogfood: https://github.com/Tigon32/archimate-js/issues/215
- Remote CI frugality follow-up: https://github.com/Tigon32/archimate-js/issues/222
- Remote CI recovery: https://github.com/Tigon32/archimate-js/issues/232
- macOS path-alias regression evidence: https://github.com/Tigon32/archimate-js/issues/161
