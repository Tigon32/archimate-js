...
import { is, isAny, normalizeArchimateType } from '../util/ModelUtil';
...
ArchimateImporter.prototype.addElement = function(viewElement, parentShape) {
  ...
  if (isAny(viewElement, [ ARCHIMATE_NODE ]) || viewElement && viewElement.$type === ARCHIMATE_NODE) {
    ...
    // MEFF node coordinates are measured from the diagram origin, including
    // nested nodes. diagram-js stores contained shapes relative to their parent.
    // Preserve the source values on businessObject.meffGeometry; this reversible
    // conversion maps them into the parent-relative canvas coordinate system.
    if (viewElement.meffGeometry && parentShape) {
      var parentOrigin = getDiagramOrigin(parentShape);
    ...
    var attrs = {
      ...
      type: normalizeArchimateType(viewElement.elementRef && viewElement.elementRef.type || viewElement.type),
    ...
    */
    ...
    logger.log('add shape to canvas');
 ...
 */
 ArchimateImporter.prototype.addConnection = function(viewElement) {
  ...
  var translate = this._translate,
    ...
    logger.log('resolved connection endpoints');

  if ((is(viewElement, ARCHIMATE_CONNECTION) || viewElement && viewElement.$type === ARCHIMATE_CONNECTION) && sourceShape && targetShape) {
    ...
    var attrs = {
      ...
      type: normalizeArchimateType(viewElement.relationshipRef && viewElement.relationshipRef.type || viewElement.type),
      businessObject: viewElement
    };
    ...
    if (getLabel(connection)) {
      this.addConnectionLabel(viewElement, connection);
    }
...
/**
 * Add a route-centered label for an imported connection.
 * MEFF connection label bounds are not currently part of the imported projection.
 */
ArchimateImporter.prototype.addConnectionLabel = function(semantic, connection) {
  var midpoint = getWaypointsMid(connection.waypoints),
      bounds = {
        x: midpoint.x - DEFAULT_LABEL_SIZE.width / 2,
        y: midpoint.y - DEFAULT_LABEL_SIZE.height / 2,
        width: DEFAULT_LABEL_SIZE.width,
        height: DEFAULT_LABEL_SIZE.height
      },
      text = getLabel(connection);
  ...
  return this._canvas.addShape(label, connection.parent);
};
...
function getDiagramOrigin(shape) {
  var origin = { x: 0, y: 0 },
      current = shape;

  while (current && current.type !== 'root') {
    origin.x += current.x || 0;
    origin.y += current.y || 0;
