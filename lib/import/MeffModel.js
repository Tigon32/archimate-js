...
export function parseMeffModel(xml, moddle) {
  ...
  parser.on('opentag', (tag) => {
    ...
    const parent = stack.length ? stack[stack.length - 1].local : null;
    ...
    if (!model && local === 'model' && tag.uri === ARCHIMATE_NS) {
      ...
      frame.field = local;
      frame.language = getAttribute(tag.attributes, '{http://www.w3.org/XML/1998/namespace}lang') ||
        getAttribute(tag.attributes, 'xml:lang') || null;
      textField = frame;
