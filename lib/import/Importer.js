...
import { MAX_VIEW_ELEMENT_DEPTH, walkViewElementTree } from './ViewTree';
        ...
        if (is(viewElement, ARCHIMATE_CONNECTION)) {
          ...
          try {
            ...
            const traversal = walkViewElementTree(
              ...
              rootShape,
              (node, parentShape) => {
                var shape = ArchimateImporter.addElement(node, rootShape);
            ...
            if (traversal.depthLimitReached) {
              warnings.push('View element nesting limit (' + MAX_VIEW_ELEMENT_DEPTH + ') reached; remaining nested elements were skipped.');
            }

            if (traversal.cycleDetected) {
              warnings.push('Cyclic view element nesting was detected; repeated nodes were skipped.');
            }
          } catch (e) {
            logger.warn('failed to import view element', {
              element: elementToString(viewElement),
      ...
      for (const connectionElement of tempered) {
        try {
          ...
          logger.warn('failed to import connection element', {
            element: elementToString(connectionElement),
            error: summarizeError(e)
          });
        }
      }
