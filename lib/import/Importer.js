    ...
    if (view.viewElements) {
      ...
      for (const viewElement of view.viewElements) {
        if (is(viewElement, ARCHIMATE_CONNECTION) ||
            viewElement && viewElement.$type === ARCHIMATE_CONNECTION) {
      ...
      for (const connectionElement of tempered) {
        try {
          ...
          if (connectionElement.meffType === 'Line' &&
              (!connectionElement.source || !connectionElement.target)) {
            addDiagnostic('IMPORT_PRESENTATION_LINE_UNRENDERED', 'A presentation line without both node endpoints could not be drawn.');
            continue;
          }
