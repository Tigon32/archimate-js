    ...
    if (view.viewElements) {
      ...
      for (const viewElement of view.viewElements) {
        if (is(viewElement, ARCHIMATE_CONNECTION)) {
          ...
          try {
            ...
            recordImportWarning(warnings, 'failed to import element', viewElement, e);
          }
        }
      }
      ...
      for (const connectionElement of tempered) {
        try {
          ...
          recordImportWarning(warnings, 'failed to import connection', connectionElement, e);
        }
      }
...
function recordImportWarning(warnings, message, element, error) {
  var elementLabel = elementToString(element);
  var errorMessage = error && error.message ? ': ' + error.message : '';
  var warning = message + ' ' + elementLabel + errorMessage;

  warnings.push(warning);
  logger.warn(message, { element: elementLabel });
}
