...
import { normalizeArchimateType } from '../../util/ModelUtil';
...
ElementFactory.prototype.createConnection = function(attrs) {
  ...
  var archimateConnection = attrs.businessObject,
      type = normalizeArchimateType(attrs.type);
  ...
  if (archimateConnection) {
    ...
    style = assign({
      ...
      lineWidth: 1
    }, archimateConnection.style && serializeStyle(archimateConnection.style));
...
ElementFactory.prototype.createShape = function(attrs) {
  ...
  attrs = attrs || {};
  attrs.type = normalizeArchimateType(attrs.type);
  ...
  if (archimateNode) {
    ...
    // Imported views often omit styles. Apply the layer palette and renderer
    // defaults first, then preserve any explicit styles from the model.
    style = attrs.type === NOTE ? assign({
      fillColor: DEFAULT_NOTE_COLOR,
      lineColor: BLACK_SHADOW,
      textAlignment: 'left',
      textPosition: 'top'
    }, archimateNode.style && serializeStyle(archimateNode.style)) : assign({
      fillColor: COLOR_LAYER_MAP.get(layer) || '#FFFFFF',
      lineColor: BLACK_SHADOW,
      textAlignment: 'center',
      textPosition: 'middle'
    }, archimateNode.style && serializeStyle(archimateNode.style));
