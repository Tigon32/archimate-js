...
import { parseMeffModel } from '../import/MeffModel';
import { parseMeffViews } from '../import/MeffView';
...
ArchimateModdle.prototype.fromXML = function(xmlStr, typeName, options) {
  ...
  const meffResult = parseMeffModel(xmlStr, this);
  if (meffResult) {
    const viewResult = parseMeffViews(xmlStr, meffResult.rootElement);
    if (viewResult.views) meffResult.rootElement.views = viewResult.views;
    meffResult.diagnostics = (meffResult.diagnostics || []).concat(viewResult.diagnostics);
    return Promise.resolve(meffResult);
  }
