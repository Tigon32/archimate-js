...
export function getLayerType(elementType) {
  elementType = normalizeArchimateType(elementType);
...
/** Normalize qualified XML concept types to renderer map keys. */
export function normalizeArchimateType(type) {
  if (typeof type !== 'string') return type;

  var normalized = type.replace(/^archimate:/, '').replace(/Relationship$/, '');
  if (normalized === 'AndJunction') return 'And Junction';
  if (normalized === 'OrJunction') return 'Or Junction';
  return normalized;
}
