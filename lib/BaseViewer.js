...
BaseViewer.prototype.importXML = async function(xml, view, isNewModel) {
  ...
  logger.log('Import XML model received', { xmlSize: getStringSize(xml) });
  ...
  try {
    ...
    logger.log('Parsed XML model', getParseDiagnostics(parseResult));
...
BaseViewer.prototype.importModel = async function(model, view, elementsById) {
  ...
  logger.log('Importing parsed ArchiMate model', getModelDiagnostics(model));
...
BaseViewer.prototype.openView = async function(viewOrId) {
  ...
  logger.log('Opening selected view', getViewDiagnostics(viewOrId));
  ...
  if (typeof viewOrId === 'string') {
    ...
    logger.log('Resolved selected view', { found: Boolean(view) });
...
BaseViewer.prototype.saveXML = function(options) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    logger.log('Preparing model for XML serialization', getModelDiagnostics(model));
    ...
    self._moddle.toXML(model, options).then(function(result) {
      ...
      try {
        ...
        logger.error('error in saveXML life-cycle listener', getErrorDiagnostics(e));
      }
...
//  return new Moddle(moddleOptions);
  ...
  moddle.ids = new Ids();
  logger.log('Created ArchiMate moddle instance');
  return moddle;
...
function getParseDiagnostics(parseResult) {
  return {
    rootType: parseResult && parseResult.rootElement && parseResult.rootElement.$type,
    rootIdPresent: Boolean(parseResult && parseResult.rootElement && parseResult.rootElement.id),
    referenceCount: getArraySize(parseResult && parseResult.references),
    warningCount: getArraySize(parseResult && parseResult.warnings),
    elementCount: getObjectSize(parseResult && parseResult.elementsById)
  };
}

function getModelDiagnostics(model) {
  var viewsList = model && model.views && model.views.diagrams && model.views.diagrams.viewsList;

  return {
    type: model && model.$type,
    idPresent: Boolean(model && model.id),
    viewCount: getArraySize(viewsList)
  };
}
...
function getViewDiagnostics(view) {
  ...
  if (typeof view === 'string') {
    return { present: true, selectorType: 'id' };
  }

  return {
    present: true,
    type: view.$type,
    idPresent: Boolean(view.id),
    elementCount: getArraySize(view.viewElements)
  };
}
