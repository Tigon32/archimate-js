...
import { resolveConceptRendererMetadata } from './concept-renderer.generated.mjs';
...
export function notationFill(layer, type) {
  const metadata = resolveConceptRendererMetadata(type);
  if (metadata?.grouping || type === 'Grouping') return notationTokens.color.grouping.fill.$value.hex.toUpperCase();
  const domain = metadata?.domain || domainNames[layer] || 'common';
  return notationTokens.color.domain[domain].fill.$value.hex.toUpperCase();
}
...
export function cornerRadius(type, height) {
  const family = resolveConceptRendererMetadata(type)?.shapeKey ||
...
export function ensureNotation(svg) {
  ...
  let sanitizedSymbols = symbols;
  let previousSymbols;
  do {
    previousSymbols = sanitizedSymbols;
    sanitizedSymbols = sanitizedSymbols.replace(/<!--[\s\S]*?-->/g, '');
  } while (sanitizedSymbols !== previousSymbols);
  const source = new DOMParser().parseFromString(
    sanitizedSymbols, 'image/svg+xml');
...
export function appendNotationIcon(parent, svg, type, width, fill) {
  if (!svg) return false;
  const symbolName = resolveConceptRendererMetadata(type)?.iconKey || type;
