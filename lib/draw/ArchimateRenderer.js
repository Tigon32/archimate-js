...
import {
  appendNotationIcon, chamferSize, cornerRadius, ensureNotation,
  linePattern, notationFill, notationStroke
} from './NotationAdapter.mjs';
...
export default function ArchimateRenderer(
  ...
  function marker(type, color) {
    var id = type + '-' + color.replace(/[^a-zA-Z0-9_-]/g, '') + '-' + rendererId;
  ...
  function renderArchimateConnection(parentGfx, connection, attrs) {
    var sourceLabel = connection.source && getLabel(connection.source) ||
      connection.source && connection.source.type || 'unnamed element';
    var targetLabel = connection.target && getLabel(connection.target) ||
      connection.target && connection.target.type || 'unnamed element';
    var relationshipLabel = getLabel(connection);
    svgAttr(parentGfx, {
      'data-export-kind': 'relationship',
      'data-export-name': connection.type + ': ' +
        (relationshipLabel ? relationshipLabel + ', ' : '') +
        'from ' + sourceLabel + ' to ' + targetLabel
    });
    ensureNotation(canvas._svg);
    var lineWidth = Number(connection.style && connection.style.lineWidth);
    // Normalize missing, non-finite, or non-positive imported widths to 1px.
    var strokeWidth = Number.isFinite(lineWidth) && lineWidth > 0 ?
    ...
    var pattern = linePattern(connection.type, strokeWidth);
    if (pattern) attrs.strokeDasharray = pattern;

    var line = drawLine(parentGfx, connection.waypoints, attrs);
    svgClasses(line).add('am-rel');
    // The notation sheet supplies defaults; inline model styles must win.
    line.style.stroke = attrs.stroke;
    line.style.strokeWidth = strokeWidth;
    if (attrs.strokeDasharray) line.style.strokeDasharray = attrs.strokeDasharray;
    return line;
  }


  function renderArchimateShape(parentGfx, shape) {
    setExportNodeName(parentGfx, shape);
    logger.log({ parentGfx, shape });
    ensureNotation(canvas._svg);

    var elementType = shape.type,
      defaultColorLayer = notationFill(shape.layer, elementType),
      borderRadius = cornerRadius(elementType, shape.height),
      explicitStyle = shape.businessObject && shape.businessObject.style;

    var attrs = {
      fill: shape.style.fillColor || defaultColorLayer,
      stroke: explicitStyle && explicitStyle.lineColor ? shape.style.lineColor : notationStroke(elementType),
    };

    var chamfer = chamferSize(elementType, shape.height);
    var figure = chamfer ? drawPath(parentGfx,
      `M ${chamfer} 0 H ${shape.width - chamfer} L ${shape.width} ${chamfer} ` +
      `V ${shape.height - chamfer} L ${shape.width - chamfer} ${shape.height} ` +
      `H ${chamfer} L 0 ${shape.height - chamfer} V ${chamfer} Z`, attrs) :
      drawRect(parentGfx, shape.width, shape.height, borderRadius, attrs);
    svgClasses(figure).add('am-shape');
    figure.style.fill = attrs.fill;
    figure.style.stroke = attrs.stroke;
    if (shape.style.lineWidth > 0) figure.style.strokeWidth = shape.style.lineWidth;
    ...
    if (appendNotationIcon(parentGfx, canvas._svg, elementType, shape.width, attrs.fill)) {
      return figure;
    }
  ...
  function setExportNodeName(parentGfx, shape) {
    var name = getLabel(shape) || 'Unnamed ' + (shape.type || 'element');
    svgAttr(parentGfx, {
      'data-export-kind': 'node',
      'data-export-name': (shape.type || 'Element') + ': ' + name
    });
  }
  ...
  this.handlers = {
    'archimate:Node': function(parentGfx, shape) {
      ...
      if (shape.businessObject && shape.businessObject.meffType === 'Container') {
        setExportNodeName(parentGfx, shape);
        var container = drawRect(parentGfx, shape.width, shape.height, 0, {
          stroke: shape.style.lineColor,
          strokeWidth: shape.style.lineWidth || DEFAULT_STROKE_WIDTH,
          fill: shape.style.fillColor || 'none'
        });
        if (getLabel(shape)) {
          renderEmbeddedLabel(parentGfx, shape, 'center-middle', shape.style.fontStyle);
        }
        return container;
      }
    ...
    'archimate:Group': function(parentGfx, element) {
      setExportNodeName(parentGfx, element);
      logger.log(element);
      ensureNotation(canvas._svg);

      var figure = drawRect(parentGfx, element.width, element.height, 0, {
        stroke: element.style && element.style.lineColor || notationStroke('Grouping'),
        strokeWidth: 1,
        strokeDasharray: '6,3',
        fill: element.style && element.style.fillColor || notationFill(null, 'Grouping'),
        pointerEvents: 'none'
      });

      var align = element.style && element.style.textAlignment + '-' + element.style.textPosition || 'center-middle',
      fontWeight = element.style && element.style.fontStyle;
    ...
    'Association': function(parentGfx, connection) {
      var color = connection.style.lineColor;
      var attrs = {};
      var relationshipRef = connection.businessObject && connection.businessObject.relationshipRef;
      var isDirected = connection.typeOption || relationshipRef && relationshipRef.isDirected;
...
ArchimateRenderer.prototype.canRender = function(element) {
  ...
  return isAny(element.businessObject, [ARCHIMATE_NODE, ARCHIMATE_CONNECTION]) ||
    element.businessObject && [ARCHIMATE_NODE, ARCHIMATE_CONNECTION].includes(element.businessObject.$type);
};
...
ArchimateRenderer.prototype.drawShape = function(parentGfx, element) {
  ...
  if (is(element.businessObject, ARCHIMATE_NODE) ||
      element.businessObject && element.businessObject.$type === ARCHIMATE_NODE) {
    type = ARCHIMATE_NODE;
  } else {
    type = element.type;
  }
