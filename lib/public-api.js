...
export async function mountViewer(options) {
  ...
  try {
    if (normalized.viewId || normalized.viewName) {
      let parsed;
      try {
        parsed = await viewer._moddle.fromXML(normalized.xml, ARCHIMATE_MODEL);
      } catch {
        throw safeError('MODEL_IMPORT_FAILED');
      }
      const view = resolveView(parsed.rootElement, normalized.viewId, normalized.viewName);
      try {
        // Parse once, resolve the selected view, and render it directly. This
        // avoids rendering the default view and switching a second time.
        await viewer.importModel(parsed.rootElement, view, parsed.elementsById);
      } catch (error) {
        if (error && SAFE_ERROR_MESSAGES[error.code]) {
          throw error;
        }
        throw safeError('VIEW_RENDER_FAILED');
      }
    } else {
      try {
  ...
  try {
    ...
    if (viewer) {
      try {
        ...
        // Preserve the static public diagnostic if cleanup itself fails.
      }
    }
    try {
      host.remove();
    } catch {
      // Host cleanup is best effort and must not expose DOM exceptions.
    }
  }
}
