...
- Grouped Dependabot minor/patch version and security PRs are also eligible for automatic draining only when created by `dependabot[bot]`, from the same repository, and their branch matches an allow-listed group from `.github/dependabot.yml`. Major or unknown Dependabot PRs remain manual.
...
- At successful task completion, do not leave an agent PR parked in Draft. Run `npm run pr:finalize`; it requires a clean `agent/*` branch, runs the full local gate, pushes exact HEAD, verifies the PR head, and marks the PR Ready. The trusted `Qualified PR drain` workflow on `main` then squash-merges automatically when all pull-request workflows for that exact HEAD are green. No per-PR auto-merge arming is required. Add the `no-auto-merge` label only when a maintainer intentionally needs to stop automatic draining.
